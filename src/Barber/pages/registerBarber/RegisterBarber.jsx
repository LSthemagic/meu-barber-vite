...
const RegisterBarber = () => {
    const [stateEstablishiment, dispatchEstablishiment] = useReducer(reducerEstablishiment, initialStateEstablishiment);
    const [stateBarber, dispatchBarber] = useReducer(reducerBarber, initialStateBarber);
    ...
    const handleEstablishiment = async (event) => {
        ...
        if (selected) {
            const selectedDataAPI = dataAPI.find((item) => item.poi.name === selected);
            ...
            if (selectedDataAPI) {
                ...
                dispatchEstablishiment({ type: "SET_LATITUDE", data: latitude });
                dispatchEstablishiment({ type: "SET_LONGITUDE", data: longitude });
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await fetch("http://localhost:3001/barberAuth/registerBarber", {
                method: "POST",
                headers: {
    ...
    const handleConfirmCode = async (event) => {
        ...
        if (code) {
            try {
                const response = await fetch("http://localhost:3001/emailAuth/auth-code", {
                    method: "POST",
                    headers: {
                        'Content-Type': "application/json"
                    },
    ...
    const handleReqEmail = async (event) => {
        event.preventDefault();
        try {
            const response = await fetch("http://localhost:3001/emailAuth/req-email-barber", {
                method: "POST",
                headers: {
                    'Content-Type': "application/json"
                },
    ...
    return (
        ...
        <div className={styles.container} style={{ backgroundImage: `url(${ImageLogin})`, backgroundSize: 'cover', minHeight: '100vh', margin: 0, padding: 0 }}>
                        ...
                        {activeStep === 0 ? (
                            <div className="form-group">
                                <form className={styles.input} onSubmit={handleReqEmail}>
                                    <h1 style={{ fontFamily: 'cursive', alignItems: "center", justifyContent: "center", display: "flex" }}>Cadastrar-se</h1>
                                    <label htmlFor="name">Nome </label>
                                    <input
                                        required
                                        placeholder="Ex: seu barber do corte"
                                        type="text"
                                        onChange={(event) => handleDispatchBarber("SET_NOME", event)}
                                    />
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => handleDispatchBarber("SET_EMAIL", event)}
                                    />
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => handleDispatchBarber("SET_PASSWORD", event)}
                                    />
                                    <button style={{ marginTop: "2%" }} className="btn btn-primary" type="submit">Próximo</button>
                                </form>
                            ...
                            <div className="form-group" >
                                <Form className={styles.input} onSubmit={handleEstablishiment}>
                                    <h3 style={{ fontFamily: 'cursive', alignItems: "center", justifyContent: "center", display: "flex" }} >Seu estabelecimento</h3>
                                    <label>Nome da barbearia</label>
                                    <Form.Control
                                        required
                                        type="text"
                                        placeholder="Ex: Barber no stylus"
                                        onChange={(e) => handleDispatchEstablishiment("SET_BARBERSHOP", e)}
                                    />
                                    ...
                                    />
                                    <small>Para garantir que sua barbearia seja encontrada, certifique-se de cadastrá-la em um aplicativo de localização.</small>
                                    <label style={{ marginTop: "2%" }}>Selecione a sua barbearia</label>
                                    <Form.Select
                                        required
                                        aria-rowcount={30}
                                        value={selected}
                                    ...
                                    >
                                        <option value="" disabled>Pesquise para selecionar</option>
                                        {dataAPI.map((item) => (
                                    ...
                                    />
                                    <button style={{ marginTop: "5%" }} className="btn btn-primary" type="submit">Próximo</button>
                                    <button style={{ marginTop: "2%" }} className="btn btn-primary" onClick={() => setActiveStep(0)}>Voltar</button>
                                </Form>
                            </div>
                        ) : (
                            <>
                                <div className="form-group">
                                    <h3 style={{ fontFamily: 'cursive', alignItems: "center", justifyContent: "center", display: "flex" }} >Verificar email</h3>
                                    <form onSubmit={handleConfirmCode} className={styles.input}>
                                        ...
                                        />
                                        <button style={{ marginTop: "2%" }} className="btn btn-primary">Confirmar</button>
                                    </form>
                                </div>
                            </>
                        )}

...
router.post("/update-clients", async (req, res) => {
    try {
        ...
        const existingClient = barber.clientes.find(client => client.email === clients.email);
        ...
        if (existingClient) {
            ...
            const barberExisting = await BarberModel.findOne({ 'clientes.email': emailClient });
            if (barberExisting) {
                // Remove o cliente da lista do barbeiro existente
                const clientIndexInExistingBarber = barberExisting.clientes.findIndex(client => client.email === emailClient);
...
router.get("/scheduled", async (req, res) => {
    try {
        ...
        const barbeiro = await BarberModel.findOne({ email: emailBarbeiro }).lean();

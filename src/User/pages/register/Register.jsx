...
const Register = () => {
    ...
    const handleReqEmail = async (event) => {
        ...
        try {
            const response = await fetch("http://localhost:3001/emailAuth/req-email", {
                method: "POST",
                headers: {
                    'Content-Type': "application/json"
                },
                    ...
                    <form className={styles.input} onSubmit={handleReqEmail}>
                        ...
                        {emailSubmit && (
                            <div>
                                ...
                                <small id="emailHelp" className="form-text text-muted">Verifique na caixa de entrada do seu email.</small><br />
                            </div>
                        )}
                        <Link className={styles.link} to={"/authenticate"} type="submit">Já tenho uma conta</Link>
                        <button style={{ marginTop: "2%" }} className="btn btn-primary" type="submit">{textButton}</button>

                        <hr />
                        <Link to="/barber/registerBarber" style={{ marginBottom: "1%" }} className="btn btn-primary" type="button">Sou Barbeiro</Link>
